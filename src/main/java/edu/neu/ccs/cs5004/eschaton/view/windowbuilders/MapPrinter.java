...
public class MapPrinter extends MapFrame {
  ...
  public static void printMap(Graphics graphics, int originX, int originY){
    ...
    Color[] colors = {Color.BLACK, Color.BLUE, Color.RED, Color.GREEN, Color.CYAN, Color.ORANGE};

    for (int distanceFromOrigin= 1; distanceFromOrigin< 6; distanceFromOrigin++) {
      ...
      for(int block = 0; block < NUMBER_OF_BLOCKS; block ++){
        ...
        graphics.fillPolygon(Hexagon.newHexagon(blockXVals[block], blockYVals[block]));
        ...
        for(int blockSize = 0;  blockSize < distanceFromOrigin-1; blockSize++){
          ...
          graphics.fillPolygon(Hexagon.newHexagon(
